...
class Similarity:
    def __init__(self, descriptor, paths, ratio=0.7, miniMatches=40, useHamming=True):
        self.descriptor = descriptor
        self.paths = paths
        self.ratio = ratio
        self.miniMatches = miniMatches
        self.distanceMethod = "BruteForce"

        if useHamming:
            self.distanceMethod += "-Hamming"

    def find_match(self, queryKps, queryDescs):
        results = {}
    ...
    def match(self, kpsA, featuresA, kpsB, featuresB):
        ...
        if len(matches) > self.miniMatches:
