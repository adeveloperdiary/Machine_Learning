...
class ANN:
    ...
    def minibatch_gd_using_rmsprop(self, derivatives):
        for l in range(1, self.L + 1):
            self.velocity["dW" + str(l)] = self.beta * self.velocity["dW" + str(l)] + (1 - self.beta) * np.square(derivatives["dW" + str(l)])
            self.velocity["db" + str(l)] = self.beta * self.velocity["db" + str(l)] + (1 - self.beta) * np.square(derivatives["db" + str(l)])

            self.parameters["W" + str(l)] = self.parameters["W" + str(l)] - (self.learning_rate / (np.sqrt(self.velocity["dW" + str(l)]) + 1e-8)) * derivatives["dW" + str(l)]
            self.parameters["b" + str(l)] = self.parameters["b" + str(l)] - (self.learning_rate / (np.sqrt(self.velocity["db" + str(l)]) + 1e-8)) * derivatives["db" + str(l)]
